...
브라우저에서 상세 API 레퍼런스를 확인할 수 있습니다.
...
`day` 보기에서 시작일이 해당 월의 마지막 날이면 표시 범위를 앞쪽으로 4일 확장하고, 종료일이 해당 월의 첫날이면 뒤쪽으로 4일 확장합니다. 일정 데이터 자체는 변경하지 않습니다.
`excludeWeekends`를 사용한 `day` 보기에서 일정 종료일이 토요일이나 일요일이면 bar는 해당 주의 금요일까지 표시합니다. 원본 종료일은 변경하지 않습니다.
객체의 `start`가 `end`보다 늦으면 해당 row는 차트 범위와 요약 계산에서 제외하고, bar 대신 날짜 오류 배지를 표시합니다.
날짜 오류 배지를 우클릭하면 오류 아이콘, 원본 기간, 상태, 진행률을 포함한 상세 메뉴를 확인할 수 있습니다.
