(function (factory) {
  ...
  var defaults = {
    ...
    contextMenuFields: null,
    contextMenuRenderer: null,
    contextMenuActions: null,
    onContextMenuOpen: null
  };
  var DEFAULT_CONTEXT_MENU_FIELDS = ['type', 'period', 'status', 'progress'];
  var CONTEXT_FIELD_BUILDERS = {
    label: {
      label: '일정명',
      value: function (row) {
        return row.label;
    ...
    type: {
      label: '유형',
      value: function (row) {
        return row.isSummary ? '요약 일정' : '작업 일정';
      }
    },
    period: {
      label: '기간',
      value: function (row, options) {
        return formatDate(row.start, options.locale) + ' - ' + formatDate(row.end, options.locale);
      }
    },
    start: {
      label: '시작일',
      value: function (row, options) {
        return row.start ? formatDate(row.start, options.locale) : '-';
      }
    },
    end: {
      label: '종료일',
      value: function (row, options) {
        return row.end ? formatDate(row.end, options.locale) : '-';
      }
    },
    status: {
      label: '상태',
    ...
    progress: {
      label: '진행률',
      value: function (row) {
        return clamp(row.progress, 0, 100) + '%';
      }
    },
    color: {
      label: '색상',
    ...
    leadTime: {
      ...
      value: function (row, options) {
        ...
        return leadTime === null ? '-' : leadTime + '일';
      }
    },
    dependencies: {
      label: '선행 작업',
      value: function (row) {
        return (row.dependencies && row.dependencies.length) ? row.dependencies.join(', ') : '-';
      }
    }
  };
  ...
  CustomGantt.prototype.bindTaskContextMenu = function () {
    ...
    this.$element.find('.cg-task-bar').on('contextmenu', function (event) {
      if (self.options.showContextMenu === false) {
        return;
      }
  ...
  CustomGantt.prototype.openTaskContextMenu = function (row, clientX, clientY) {
    var self = this;
    var opts = this.options;
    var source = row.source || row;
    var context = buildContextMenuContext(row);
    var items = resolveContextMenuItems(row, opts, context);
    var actions = resolveContextMenuActions(row, opts, context);
    ...
    items.forEach(function (item) {
      ...
      if (item.type === 'separator') {
        $menu.append($('<div class="cg-context-separator">'));
        return;
      }

      $menu.append(renderContextRow(item.label, item.value));
    });
    ...
    if (actions.length) {
      ...
      actions.forEach(function (action) {
        var $action = $('<button class="cg-context-action" type="button" role="menuitem">')
          .toggleClass('is-danger', !!action.danger)
          ...
          .on('click', function (event) {
            ...
            if (typeof action.onClick === 'function') {
              action.onClick(source, $.extend({}, context, { event: event }));
            }
    ...
    if (typeof opts.onContextMenuOpen === 'function') {
      opts.onContextMenuOpen($menu, source, context);
    }
  ...
  CustomGantt.prototype.applyDependencySelection = function () {
    ...
    var defaultArrowUrl = 'url(#' + this.dependencyArrowId + ')';
    var activeArrowUrl = 'url(#' + this.dependencyArrowId + '-active)';
    ...
    if (!activeRowId) {
      $lines.removeClass('is-dimmed is-highlighted').attr('marker-end', defaultArrowUrl);
      return;
    }
    ...
    $lines.each(function () {
      ...
      $line
        ...
        .attr('marker-end', isConnected ? activeArrowUrl : defaultArrowUrl);
    });
  ...
  function buildDependencyEdges(visibleRows) {
    ...
    visibleRows.forEach(function (row) {
      (row.dependencies || []).forEach(function (dependencyId) {
        ...
        if (fromRow && fromRow !== row && fromRow.start && fromRow.end && row.start && row.end) {
          edges.push({ from: fromRow, to: row });
        }
      });
    });
  ...
  function renderDependencyLines(edges, rowIndexMap, rowCount, units, unitWidth, rowHeight, arrowId) {
    ...
    edges.forEach(function (edge) {
      ...
      var toX = toMetrics.left;
      var path = createSvgElement('path');

      path.setAttribute('class', 'cg-dependency-line');
      path.setAttribute('data-from-row', edge.from.id);
      path.setAttribute('data-to-row', edge.to.id);
      path.setAttribute('marker-end', 'url(#' + arrowId + ')');
      path.setAttribute('d', buildDependencyLinePath(fromX, fromY, toX, toY, rowHeight));
  ...
  function buildContextMenuContext(row) {
    return {
      row: row,
      type: row.type,
      status: row.status,
  ...
  function resolveContextMenuItems(row, options, context) {
    if (typeof options.contextMenuRenderer === 'function') {
      var rendered = options.contextMenuRenderer(context.source, context);

      if (Array.isArray(rendered)) {
        return normalizeContextMenuItems(rendered);
      }
    }

    var fields = (Array.isArray(options.contextMenuFields) && options.contextMenuFields.length)
      ? options.contextMenuFields
      : DEFAULT_CONTEXT_MENU_FIELDS;

    return fields.map(function (field) {
      return resolveContextField(field, row, options, context);
    }).filter(Boolean);
  }
  ...
  function normalizeContextMenuItems(items) {
    return items.filter(Boolean).map(function (item) {
      ...
      return {
        type: 'info',
        label: item.label,
        value: formatContextFieldValue(item.value)
      };
    });
  }

  function resolveContextField(field, row, options, context) {
    if (field && field.type === 'separator') {
      return { type: 'separator' };
    }

    if (typeof field === 'string') {
      var builder = CONTEXT_FIELD_BUILDERS[field];
      ...
      return {
        type: 'info',
        label: builder.label,
        value: formatContextFieldValue(builder.value(row, options))
      };
    }
    ...
    if (field && typeof field === 'object') {
      ...
      if (typeof field.value === 'function') {
        value = field.value(context.source, context);
      } else if (field.key) {
        value = (context.source && context.source[field.key] !== undefined) ? context.source[field.key] : row[field.key];
      } else {
        value = field.value;
      }
      ...
      return {
        type: 'info',
        label: label,
        value: formatContextFieldValue(value)
      };
    }
  ...
  function formatContextFieldValue(value) {
  ...
  function resolveContextMenuActions(row, options, context) {
    var actions = options.contextMenuActions;

    if (typeof actions === 'function') {
      actions = actions(context.source, context);
    }
